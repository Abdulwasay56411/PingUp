            ...
            <div className="bg-white rounded-lg hidden lg:block shadow-md w-80 h-52 my-5 p-2">
              ...
              <div>
                {recentMessage.map((showMessage) => {
                  return (
                    <div key={showMessage.id} className="flex justify-between items-center px-2 my-4">
                      <div className="flex gap-2 items-center">
                        <img className="w-8 h-8" src={showMessage.imgae} />
                        <div>
                          <h1 className="font-medium text-[12px] text-[#1D293D] ">{showMessage.name}</h1>
                          <p className="text-[12px] text-[#6A7282]">{showMessage.para}</p>
                        </div>
                      </div>
                      <div>
                        <h2 className="text-[10px] text-[#90A1B9] font-normal">{showMessage.seen}</h2>
                      </div>
                    </div>
                  );
                })}
